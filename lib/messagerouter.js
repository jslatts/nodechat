...
(function () {
    ...
    function parseControlWordsFromChat(message, fn) {
        ...
        if (topic) {
            ...
            return fn('topic:default', topic, message.data);
        }
        ...
        switch(type){
            case 'direct':
                ...
                createChat(message, client, function (err, chatModelSender) {
                    ...
                    winston.info('[handleChat][createChat][fn] publishing chat to user:' + client.user + ':directs.' + topic + ' - ' + chatModelSender.xport());
                    client.rc_sub.subscribe('user:' + client.user + ':directs' + topic, function () {
                        client.rc_pub.publish('user:' + client.user + ':directs.' + topic, chatModelSender.xport());

                        winston.info('[handleChat][createChat][fn] saving chat to user:' + client.user + ':directs - ' + chatModelSender.xport());
                        rc.rpush('user:' + client.user + ':directs.' + topic, chatModelSender.xport());
                    });
                    ...
                    createChat(message, client, function (err, chatModelReceiver) {
                        ...
                        winston.info('[handleChat][createChat][fn] publishing chat to user:' + topic + ':directs.' + message.topic + ' - ' + chatModelSender.xport());
                        client.rc_sub.subscribe('user:' + topic + ':directs' + message.topic, function () {
                            client.rc_pub.publish('user:' + topic + ':directs.' + message.topic, chatModelSender.xport());

                            winston.info('[handleChat][createChat][fn] saving chat to user:' + topic + ':directs.' + message.topic + ' - ' + chatModelReceiver.xport());
                            rc.rpush('user:' + topic + ':directs.' + message.topic, chatModelReceiver.xport());
    ...
    function topicGetAllMessages(message, client, fn) {
        channelmanager.subscribeClientToChannel(client, message.topic, function () {
            winston.info('[topicGetAllMessages] retreiving history for topic: ' + message.topic);
    ...
    exports.handleMessage = function(message, client, fn) {
        ...
        switch(message.event) {
            ...
            case 'topic:unsubscribe':
                channelmanager.unsubscribeClientFromChannel(client, message.topic, fn);
                break;
        }
    };
})()
